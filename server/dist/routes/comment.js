...
// 点赞评论
router.post("/:id/like", auth_1.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
...
// 取消点赞
router.delete("/:id/like", auth_1.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
...
// 回复评论
router.post("/:id/replies", auth_1.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { content, replyToUserId } = req.body;
        const userId = req.user.id;
        if (!content) {
            throw new errors_1.ApiError(400, "缺少回复内容");
        }
