            ...
            >
              ...
              >
                <div class="comment-input-wrapper">
                  ...
                  >
                    <EmojiPicker
                      @select="handleEditEmojiSelect"
                      @close="showEditEmojiPicker = false"
                    />
                  </div>
                  <div
                    class="comment-image-preview"
                    v-if="editCommentImages.length > 0"
                  >
                    <div
                      v-for="(image, index) in editCommentImages"
                      :key="index"
                      class="preview-item"
                    >
                      <img :src="image.url" alt="预览" class="preview-img" />
                      <div
                        class="preview-remove"
                        @click="removeEditCommentImage(index)"
                      >
                        <el-icon><Close /></el-icon>
                      </div>
                    </div>
...
const editCommentImages = ref<{ url: string; file?: File }[]>([]);
const showEditEmojiPicker = ref(false);
const originalEditImages = ref<string[]>([]);
...
const commentImages = ref<{ url: string; file: File }[]>([]);
const showReplyEmojiPicker = ref(false);
const replyImages = ref<{ url: string; file: File }[]>([]);
...
const handleSubmitComment = async () => {
  ...
  try {
    ...
    const postId = route.params.id as string;
    const newImages = commentImages.value.map((item) => item.file);
...
const handleImageUpload = (event: any) => {
  ...
  const maxRemaining = 9 - commentImages.value.length;
  const filesToAdd = Array.from(files) as File[];
  const slicedFiles = filesToAdd.slice(0, maxRemaining);

  slicedFiles.forEach((file: File) => {
...
const handleEditComment = (comment: any) => {
  ...
  originalEditImages.value = comment.images || [];
  editCommentImages.value = (comment.images || []).map((img: string) => ({
    url: getImageUrl(img),
  }));
};
...
const handleEditImageUpload = (event: any) => {
  ...
  const maxRemaining = 9 - editCommentImages.value.length;
  const filesToAdd = Array.from(files) as File[];
  const slicedFiles = filesToAdd.slice(0, maxRemaining);
...
const removeEditCommentImage = (index: number) => {
  editCommentImages.value.splice(index, 1);
};

const submitEditComment = async (commentId: number) => {
  if (
    !editCommentContent.value.trim() &&
    editCommentImages.value.length === 0
  ) {
    ElMessage.warning("请输入评论内容或上传图片");
    return;
  }
  ...
  try {
    ...
    const newImages = editCommentImages.value
      .filter((item) => item.file)
      .map((item) => item.file!);

    const currentExistingImages = editCommentImages.value
      .filter((item) => !item.file)
      .map((item) => {
        if (item.url.startsWith("/")) {
          return item.url;
        }
        const match = item.url.match(/\/uploads\/comments\/[^/]+$/);
        return match ? match[0] : item.url;
      });

    const removedImages = originalEditImages.value.filter(
      (img) => !currentExistingImages.includes(img),
    );
...
const handleReplyImageUpload = (event: any) => {
  const files = event.target.files;
  if (!files || files.length === 0) return;

  const maxRemaining = 9 - replyImages.value.length;
  const filesToAdd = Array.from(files) as File[];
  const slicedFiles = filesToAdd.slice(0, maxRemaining);
...
const handleSubmitReply = async (commentId: number) => {
  ...
  if (!content.trim() && replyImages.value.length === 0) {
  ...
  try {
    ...
    const newImages = replyImages.value.map((item) => item.file);
...
const previewImage = (imagePath: string) => {
  ...
  if (previewWindow) {
    previewWindow.document.write(`
      ...
      <head>
        <style>
          body { margin: 0; display: flex; justify-content: center; align-items: center; height: 100vh; background: rgba(0,0,0,0.9); }
...
.edit-comment-form .comment-input-wrapper {
  position: relative;
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
